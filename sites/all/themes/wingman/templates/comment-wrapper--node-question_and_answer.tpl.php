...
<section id="comments" class="comments <?php print $classes; ?>"<?php print $attributes; ?>>
  ...
  <?php if ($node->type != 'question_and_answer'): ?>
	  <?php if ($comments && $node->type != 'forum'): ?>
	    <h2 class="comments__title title"><?php print t('Comments'); ?></h2>
	  <?php endif; ?>
  <?php endif; ?>	
  <?php if ($node->type = 'question_and_answer'): ?>
    	<h2 class="comments__title title"><?php print t('Answers'); ?></h2>
  <?php endif; ?>
  ...
  <?php if ($comment_form): ?>
	...
	<?php endif; ?>
	<?php if ($node->type = 'question_and_answer'): ?>
		<h2 class="comments__form-title title comment-form"><?php print t('Answer this Question'); ?></h2>
	<?php endif; ?>
